...
def mock_rabbitmq_uri():
    rabbitmq_uri = os.getenv('RABBITMQ_URI', 'default_value_for_testing')
    with patch.object(Config, 'RABBITMQ_URI', rabbitmq_uri):
        yield
...
def test_parse_rabbitmq_url(mock_blocking_connection):
    ...
    expected_url = os.getenv('RABBITMQ_URI', 'default_value_for_testing')
